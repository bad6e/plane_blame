...
RSpec.describe Departure, type: :model do
  ...
  it "returns the number of departures" do
    expect(Departure.total_departures).to eq(2)
  end

  it "returns the total number of ontime flights" do
    expect(Departure.total_on_time_flights).to eq(1)
  end

  it "returns the total number of late flights" do
    expect(Departure.total_late_flights).to eq(1)
  end

  it "returns the total ontime flights per airline" do
    expect(Departure.total_on_time_number_per_airline(@airline_one.id)).to eq(1)
  end

  it "returns the total flights per airline" do
    expect(Departure.total_flights_per_airline(@airline_one.id)).to eq(2)
  end
